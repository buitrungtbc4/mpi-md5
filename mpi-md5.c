...
void str2digest_md5(const char *str, unsigned char *digest);
...
void num_pass2str(int num_pass, int len, char* str_pass){
    int i = len - 1;
    int j;
    int index;
    for(j = 0; j < len; j ++){
...
void bruteForce(int process_size, int len, int rank){
    ...
    char str_pass[len];
    for(i = from; i < to; i ++){
        num_pass2str(i, len, str_pass);
        str2digest_md5(str_pass, testDigest);
